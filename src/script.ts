...
let people: Person[] = [
  { name: "Sivatheking", id: 1, phonenumber: 9361213535 },
  { name: "Sanjay", id: 2, phonenumber: 9876543210 },
  { name: "Addwin", id: 3, phonenumber: 9123456780},
];
const todoRef = document.getElementById("tablebody") as HTMLInputElement;
const InputRef = document.getElementById("todoInput") as HTMLInputElement;
const InputRef1 = document.getElementById("todoInput1") as HTMLInputElement;
const AddRef = document.getElementById("Add") as HTMLInputElement;
let addid = 0;
window.addEventListener("load", () => {
  loadtheuser();
});
const loadtheuser = (): void => {
  let body = "";
  for (let todo of people) {
    body += `<tr>
        <th>${todo.id}</th>
        <td>${todo.name}</td>
